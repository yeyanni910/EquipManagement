...
public class MainActivity extends BaseActivity implements TopBarView.onTitleBarClickListener {
    ...
    @OnClick({R.id.main_my_task,R.id.main_launch_process,R.id.main_process_launched,R.id.main_my_approval})
...
//                startActivity(new Intent(this,LaunchProcessActivity.class));
                loadWebpage("发起流程","flowBase/user-list?");
                break;
            default:
                break;
        }
    ...
    private void loadWebpage(String titleContent,String partUrl){
        ...
        String userPsd = UserUtils.getUserCache().getUserPsd();
    ...
    private void getFlowCount(){
        ...
        MobApi.getFlowCount(username, userMsg, 0x002, new MyCallBack() {
            @Override
